...
import { type DomainRoute, generateCaddyfile, reloadCaddy } from "./caddy.js";
import {
...
export async function syncCaddyConfig(
	...
	try {
		...
		const caddyfilePath =
			process.env.CADDYFILE_PATH || path.join(caddyDir, "Caddyfile");
		...
		if (!reloadResult.success) {
			logStream?.(
				`[Caddy] Notice: Proxy reload skipped or inactive (${reloadResult.error || "not running"})\n`,
			);
		} else {
		...
		logStream?.(
			`[Caddy] Notice: Could not sync Caddy configuration: ${err.message}\n`,
		);
	}
}
...
export async function executeDeployment(
	...
	appendAndBroadcast(
		`Application: ${app.name} (${app.appType.toUpperCase()})\n`,
	);
	...
	try {
		...
		try {
			await executeCommandWithLogs(
				"docker",
				["network", "create", "nimploy-network"],
				{
					...runnerOptions,
					onLog: undefined,
				},
		...
		if (app.appType === "image") {
			...
			appendAndBroadcast(
				`==> [2/3] Cleaning up previous container '${containerName}'...\n`,
			);
			...
			appendAndBroadcast(
				`==> [3/3] Starting container '${containerName}'...\n`,
			);
			const dockerRunArgs = [
				"run",
				"-d",
			...
			if (!app.repositoryUrl) {
				throw new Error(
					"Git repository URL is required for Dockerfile deployment",
				);
			}
			...
			appendAndBroadcast(
				`==> [1/4] Preparing Git repository: ${app.repositoryUrl} (${app.branch || "main"})...\n`,
			);
			if (!fs.existsSync(path.join(appRepoDir, ".git"))) {
				...
				await executeCommandWithLogs(
					"git",
					["reset", "--hard", `origin/${app.branch || "main"}`],
					{
						...runnerOptions,
						workingDir: appRepoDir,
					},
			...
			appendAndBroadcast(
				`==> [2/4] Building Docker image '${imageTag}' from ${app.dockerfilePath || "Dockerfile"}...\n`,
			);
			...
			appendAndBroadcast(
				`==> [3/4] Cleaning up previous container '${containerName}'...\n`,
			);
			await executeCommandWithLogs("docker", ["rm", "-f", containerName], {
				...runnerOptions,
				onLog: undefined,
			...
			appendAndBroadcast(
				`==> [4/4] Starting container '${containerName}'...\n`,
			);
			const dockerRunArgs = [
				"run",
				"-d",
			...
			if (!app.repositoryUrl) {
				throw new Error(
					"Git repository URL is required for Docker Compose deployment",
				);
			}
			...
			appendAndBroadcast(
				`==> [1/2] Preparing Git repository: ${app.repositoryUrl} (${app.branch || "main"})...\n`,
			);
			if (!fs.existsSync(path.join(appRepoDir, ".git"))) {
				await executeCommandWithLogs(
					"git",
			...
			appendAndBroadcast(
				`==> [2/2] Starting Docker Compose stack (${app.composePath || "docker-compose.yml"})...\n`,
			);
