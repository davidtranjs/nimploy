					...
					const [app] = await db
						...
						.where(
							or(
								...
								eq(schema.applications.slug, appId),
							),
						);
						...
						if (ws.readyState === WebSocket.OPEN) {
							ws.send(
								JSON.stringify({
									type: "container-log",
									appId,
									data: `Error connecting to container logs: ${err.message}\n`,
								}),
							);
						}
					});
						...
						if (code !== 0 && !isClosed && ws.readyState === WebSocket.OPEN) {
							ws.send(
								JSON.stringify({
									type: "container-log",
									appId,
									data: `Container "${containerName}" is not running or has not been deployed yet.\n`,
								}),
							);
						}
					});
					...
					if (ws.readyState === WebSocket.OPEN) {
						ws.send(
							JSON.stringify({
								type: "container-log",
								appId,
								data: `Failed to stream container logs: ${err.message}\n`,
							}),
						);
					}
				}
