...
describe("Deployment Service", () => {
	...
	describe("Utility functions", () => {
		it("should parse environment variables from JSON and KEY=VALUE format", () => {
			...
			expect(parseEnvVars(jsonEnv)).toEqual({
				PORT: "8080",
				NODE_ENV: "production",
			});
		...
		it("should generate safe docker container names", () => {
			expect(
				getContainerName({ id: "app-123", name: "My Web Application!" }),
			).toBe("nimploy-my-web-application-app-123");
			expect(getContainerName({ id: "xyz", name: "---abc---" })).toBe(
				"nimploy-abc-xyz",
			);
			expect(getContainerName({ id: "9Mb-g0SD73", name: "xyz" })).toBe(
				"nimploy-xyz-9Mb-g0SD73",
			);
		});
	});
