...
describe("In-Process JobQueue", () => {
	...
	it("should process jobs sequentially in FIFO order", async () => {
		...
		const rowB = sqlite
			.prepare("SELECT status FROM jobs WHERE id = 'job-2'")
			.get() as { status: string };
	...
	it("should handle failed jobs by setting status to FAILED", async () => {
		...
		const row = sqlite
			.prepare("SELECT status FROM jobs WHERE id = 'job-fail'")
			.get() as { status: string };
    ...
    `);
		...
		const row = sqlite
			.prepare("SELECT status FROM jobs WHERE id = 'job-crash'")
			.get() as { status: string };
	...
	it("should pass log stream callback to handler", async () => {
		const logs: string[] = [];
		queue.registerHandler("log-job", async (_payload, logStream) => {
		...
		await queue.idle();
		const row = sqlite
			.prepare("SELECT status FROM jobs WHERE id = 'job-log'")
			.get() as { status: string };
