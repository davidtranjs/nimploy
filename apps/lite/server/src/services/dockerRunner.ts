...
export function streamContainerLogsProcess(containerName: string, tail = 100) {
...
export async function getContainerLogs(
	...
	return new Promise((resolve) => {
		...
		proc.on("close", (code) => {
			if (code !== 0 && !output) {
				resolve(
					`Container "${containerName}" is not running or has not been deployed yet.\n`,
				);
			} else {
				resolve(output);
			}
