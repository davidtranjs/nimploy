...
import { generateCaddyfile, reloadCaddy, type DomainRoute } from "../src/services/caddy";
...
describe("Caddy Dynamic Proxy Configuration", () => {
  ...
  it("should generate Caddyfile without email when adminEmail is not provided", () => {
    const routes: DomainRoute[] = [
      { host: "demo.example.com", containerName: "demo-app", containerPort: 3000, httpsEnabled: true },
    ];
  ...
  describe("reloadCaddy", () => {
    ...
    it("should return error message when caddy reload fails", async () => {
      vi.mocked(exec).mockImplementation(((cmd: string, callback: any) => {
        callback(new Error("Caddy process not running"), { stdout: "", stderr: "" });
        return {} as any;
      }) as any);
